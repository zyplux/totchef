...
- **`snap_cook.py`** (`SnapCook`) — `[snap].packages`; for browsers Ubuntu ships only as snaps (`firefox`, `chromium` — the apt packages of those names are transitional debs that just pull the snap). Install/refresh decision driven by a single up-front `snap list` parse; entries run **sequentially** (snapd serializes behind a global lock). Install failure → `hard_fail`, refresh failure → `soft_fail`. Strictly-confined snaps only (no `--classic`). `needs_root = true` (`snap install` requires root); `depends_on = []`. Note: snap confinement means a host-side VA-API driver isn't visible inside the sandbox, so this cook can't make a snap browser hardware-decode video — it only installs.
